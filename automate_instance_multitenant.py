...
def instance_creation():
    ...
    net_qty = input("*** Please enter the number of networks to be associated: ")
    net_id = input("Please enter the network id or ids to be associated separated by comma: ")
    if ',' in net_id:
        net_id_list = net_id.split(',')
    print('\n')
    server_create_cmd = 'openstack server create --flavor m1.tiny --image cirros-0.5.2-x86_64-disk --key-name rmkey '
    cmd_iter = 0
    net_list_iter = 0
    while cmd_iter != int(net_qty):
        server_create_cmd += ' --network {}'.format(net_id_list[net_list_iter])
        cmd_iter += 1
        net_list_iter +=1
        
    server_create_cmd += ' {}'
    print(server_create_cmd)
    create_net = os.popen(server_create_cmd.format(instance_name)).read()
    print('\n*** Instance created! ')
